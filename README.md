...
| `HELM_EXTRA_OPTS`| Append string to helm install command, so that it is easier to pass it in CI pipelines, for example to override values such as private repo credentials, you can define it also in config file | `` |
...
#### Passing extra parameters to Helm

Sometimes you do not want to store certain data in repo itself, but still need to adjust the setting when invoking
tests in CI pipeline, for example to override variables such as passwords.

You can just export `HELM_EXTRA_OPTS` var or set it in config file:

```shell
export HELM_EXTRA_OPTS="--set imageCredentials.password=SECRET_PASS"
```

or more complex, pass additional vars in `override.yaml` config file, which can be earlier generated by other tools:

```shell
export HELM_EXTRA_OPTS="--f override.yaml"
```

If you run it in docker you must pass env var into docker process itself:

```shell
export HELM_EXTRA_OPTS="-f private-repo.yaml"
export CHART_TESTING_IMAGE="chart-testing:local-dev"

docker run -it --network host -v "$HOME:$HOME:ro" -v "$HOME:/root:ro" -v "$(pwd):/workdir" \
    --workdir /workdir \
    -e HELM_EXTRA_OPTS="${HELM_EXTRA_OPTS}" \
    "${CHART_TESTING_IMAGE}" \
    chart_test.sh --no-lint --config .config-priv

```

Notice:

- `HELM_EXTRA_OPTS` will NOT be displayed in the console for security reasons
- env var can override any other parameters, because it is added at the end of `helm install`
- what can be passed to `helm init` - see helm docs.
